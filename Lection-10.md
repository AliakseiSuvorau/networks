...
4) Когда второе устройство решает закрыть соединение, оно шлет первому пакет `FIN`. содержащее поля `SEQ` и `ACK`, и переходит в состояние `LAST_ACK`.
5) Первое устройство после получения пакета `FIN` шлет в ответ пакет `ACK`. В нем содержется поле `SEQ`, равное значению поля `ACK` из полученного пакета `FIN`,
