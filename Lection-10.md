...
* [TCP](https://github.com/AliakseiSuvorau/networks/blob/master/Lection-10.md#tcp-transmission-control-protocol)
  * Общая информация
...
1) Отправленные и подтвержденные
2) Отправленные, но не подтвержденные
3) Не отправленные, но получатель уже готов их получить (влазят в окно)
4) Не отправленные, за пределами окна
...
1+2) Полученные и подтвержденные
3) Не полученные, но источник может их отправить (внутри окна)
4) Не полученные и не подтвержденные, за пределами окна
...
* *Решение*:
  * Максимально допустимый (по стандарту) размер дейтаграммы, при котором гарантированно не произойдет фрагментации - 576 октет (соответственно у сегмента - 536)
