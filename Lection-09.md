...
* 16 бит - нули (выравнивание) или домен маршрутизации (тег, связанный с тем, пришел ли пакет от внешнего по отношению к текущей автономной системе роутера)
* RIP Entry (маршрутные записи)

#### RIP v1 (Вид записей, после заголовка общего вида длиной 32 бита)

* 16 бит - address family id (определяет, какой протокол был использован. Раньше предпологалось, что RIP будет использоваться не только с IPv4)
...
* 16 бит - route tag (тег, связанный с пометкой маршрута как внешнего по отношению к текущей автономной системе)
* 32 бит - ip-адрес
...
> [!NOTE]
> После заголовка идут записи о векторе расстояний. Если используется алгоритм аутентификации, то в качестве первой записи будет стоять аутентификация `0xFFFF`.
> Если же используется алгоритм криптографической аутентификации, то в качестве первой и последней записей будут стоять строки, связанные с этим алгоритмом.
...
Введем некоторые формальные определения:
...
> [!IMPORTANT]
> Заголовки пересылаются только hello-пакетами, далее - уже полные записи.
...
* Autonomous System Summary Links (Type 4)
  * Состояние каналов ABSR
  * Рассылается ABR
  * Содершит информацию о наличии в области ABSR
* Autonomous System External Links (Type 5)
...
> [!IMPORTANT]
> Тот, кто обнаружил изменения в сети, отсылает мультикастом всем, включая DR. От DR информацию получают все. Также все рассылают инфу в другие сети (через интерфейсы,
> не находящиеся в нашей сети). DR здесь для того, чтобы не было ситуации, когда каждый роутер, который получил информацию, не рассылал его всем. 
...
* Прозрачный механизм для преобразования адресов из одной группы в адреса другой группы
* Базовый стандарт: RFC 3022
* В качестве NAT-устройства может выступать, к примеру, роутер
* При прохождении через NAT внутренний адрес будет подменен на внешний
* Нужен для того, чтобы из частной сети (в которой имеем не маршрутизируемый в интернет адрес) посылать запросы в интернет
* NAPT - подменяет еще и порт (далее - NAT)
* Реализуется через таблицу преобразований на шлюзе
* Удаляем из таблицы по исчтечении некоторог времени
  * В протоколах с установлением соединения (TCP) обычно таймаут большой (порядка 1000 минут)
  * В протоколах без установления соединения (UDP) обычно таймаут маленький (порядка 1 минуты)

### Задачи NAT-шлюза

* Преобразование адресов и портов и, как следствие, пересчет контрольной суммы

### Методы трансляции

* Full Cone
  * связка активна для любого внешнего трафика
  * используется в распределенных системах: один узел принял запрос, а второй сформировал и отправил ответ
  * любой может посылать на этот адрес

* (Address) Restricted Cone
  * проверяем внешний ip-адрес из заголовка

* Port Restricted Cone
  * проверяем внешний ip-адрес и порт
  * с одного адреса могут общаться несколько приложений

* Symmetric NAT
  * общаемся с несколькими приложениями извне

> [!NOTE]
> Если с клиента приложение работает с двумя и более серверами, то в первых трех случаях одному адресу внутреннему будет соответствовать один адрес+порт шлюза
> для взаимодействия со всеми серверами. В случае Symmetric NAT будут открыты отдельные соединения.

> [!NOTE]
> Если клиент инициирует подключение к серверу ("против направления" работы шлюза), то делаем статическую конфигурацию, т.е. говорим, что конкретный порт остается
> открытым, и пересылаем с него сообщенеи на какой-то конкретный ip-адрес и порт (там это все счастье будет слушать наш сервер).

> [!IMPORTANT]
> * Нельзя хардкодить номера портов и ip-адреса в приложениях, т.к. NAT-шлюз может их подменить по истечении таймаута.
> * Стараемся использовать один сокет.
