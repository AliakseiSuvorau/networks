...
> *Решение*:
>
> Для начала посчитаем количество битов, нужное для того, чтобы выдать всем устройствам подсети адрес: $\lceil\log_2N\rceil$, где $N$ - количество хостов.
...
> [!NOTE]
> * При ошибке получения фрагментированной дейстграммы посылается icmp-сообщения только для одного фрагмента.
...
* Назначаемые
  * Системные: 0 - 1023
  * Пользовательские: 1024 - 49151 (Сами можем назначить один из этих портов)
* Динамические: 49152 - 65535 (Если, к примеру, хотим просто послать UDP-сообщение, но нет необходимости получать на него ответ, то мы не указываем порт.
  Однако поле порта должно быть заполнено, и ОС сама выдает нам како-то свободный порт из этого диапазона)
...
* Минимальный - 8
* Максимальный - $2^{16}-1-8=65527$, но на деле из этой цифры еще вычитается длина ip-заголовка, т.е. получается 65507
...
* Checksum coverage (16 бит) - указывает, какие данные защищены контрольной суммой, в октетах. Минимальное значение - 8 (длина заголовка)
...
> [!IMPORTANT]
> Наши NAT-устройства умеют хорошо работать сос тандартными транспортными протоколами типа TCP и UDP, но с другими у них могут быть проблемы.
> Для решения этих проблем мы просто инкапсулируем эти более продвинутые протоклы внутрь того же UDP.
-------------------------------------------------
## Multicasting
**Вопрос**: Как формировать multicast-адресацию на уровне mac-адресов?

**Ответ**: Трансляция multicast-адресов:
...
* IGMPv2(1997) - RFC2236. Появилась возможность посылать Leave Message. *Наиболее часто используемый*.
...
* Роутер с помощью multicast-сообщения опрашивает все хосты в локальной сети (224.0.0.1), есть ли желающие получать хоть какой-то multicast-трафик
  * Если в сети несколько маршрутизаторов, то опрашивать будет только один, чтобы не было огромного количества сообщений. Это маршрутизатор с наименьшим ip-адресом.
...
* Timeout - 260 секунд (если не было активных ответов из группы, то вычеркиваем ее)
  * Если кто-то из группы уже сообщил роутеру, что multicast-трафик в эту сеть передавать нужно, то никто другой из сети не будет слать сообщение об этом.
    Дело в том, что хост роутеру шлет тоже multicast-сообщение (хотя на канальном уровне это как broadcast), и все другие хосты его тоже слышат
* Роутрер посылает несколько сообщений (по умолчанию - 2) с поправкой на ненадежность сети
* На хосте:
  ...
  * Предположим, что какое-то приложение только что стартовало в нашей сети, и оно еще не знает, согласился ли кто-то из сети на прием multicast-сообщений.
    Тогда оно само шлет сообщение об этом роутеру. Такое сообщение называется membership report
...
* Если коммутатор достаточно умный, то он может подслушивать общение хостов и роутеров и использовать полученную инфу для более умной пересылки.
  * *Проблема*: У нас всегда отвечает о желании прослушивать multicast-трафик первый из группы. Он и будет получать все сообщения. А как узнать, хотят ли остальные хосты получать этот трафик?
...
* Подслушивание снижает нагрузку на сетевую архитектуру.
...
> Не понятно, как роутеры будут между собой определять, нужно им пересылать multicast-трафик, или нет. Но есть "костыльное" решение, применимое в некоторых ситуациях -
> IGMP Proxy.
